<?php
/**
 * Plugin Name:       The Events Calendar Extension: Speaker Linked Post Type
 * Plugin URI:        https://theeventscalendar.com/knowledgebase/linked-post-types/
 * GitHub Plugin URI: https://github.com/mijobrands/tribe-ext-speaker-linked-post-type
 * Description:       Create and link a speaker custom post type to the events custom post type
 * Version:           1.0.2
 * Extension Class:   Tribe__Extension__Speaker_Linked_Post_Type
 * Author:            Modern Tribe, Inc.
 * Author URI:        http://m.tri.be/1971
 * License:           GPL version 3 or any later version
 * License URI:       https://www.gnu.org/licenses/gpl-3.0.html
 * Text Domain:       tribe-ext-speaker-linked-post-type
 */
...
if ( ! function_exists( 'tribe_ext_is_event_speaker' ) ) {
	/**
	 * Conditional tag to check if the current page is a single speaker page.
	 *
	 * @return bool
	 **/
	function tribe_ext_is_event_speaker() {
		global $wp_query;

		$tribe_ext_is_event_speaker = ! empty( $wp_query->tribe_ext_is_event_speaker );

		return apply_filters( 'tribe_ext_query_is_event_speaker', $tribe_ext_is_event_speaker );
	}
}
...
if (
	...
	&& ! class_exists( 'Tribe__Extension__Speaker_Linked_Post_Type' )
) {
	/**
	 * Extension main class, class begins loading on init() function.
	 */
	class Tribe__Extension__Speaker_Linked_Post_Type extends Tribe__Extension {
		 ...
		 * @return string
		 */
		const POST_TYPE_KEY = 'tribe_ext_speaker';
		...
		public function init() {
			...
			load_plugin_textdomain( 'tribe-ext-speaker-linked-post-type', false, basename( dirname( __FILE__ ) ) . '/languages/' );
			...
			// Single Speaker page: Handling the No Events Found situation.
		...
		public function add_filter_to_filterbar() {
			if ( class_exists( 'Tribe__Events__Filterbar__Filters__Speaker' ) ) {
				new Tribe__Events__Filterbar__Filters__Speaker( $this->get_post_type_label(), self::POST_TYPE_SLUG );
			}
		}
		 ...
		 *
		 * @see Tribe__Extension__Speaker_Linked_Post_Type::get_custom_field_labels()
		 * @see sanitize_key()
		 *
		 * @return array
		 */
		...
		public function register_our_post_type() {
			$labels = array(
				'name'                    => esc_html_x( 'Speakers', 'Post type general name', 'tribe-ext-speaker-linked-post-type' ),
				'singular_name'           => esc_html_x( 'Speaker', 'Post type singular name', 'tribe-ext-speaker-linked-post-type' ),
				'singular_name_lowercase' => esc_html_x( self::POST_TYPE_SLUG, 'Post type singular name', 'tribe-ext-speaker-linked-post-type' ),
				// not part of WP's labels but is required by Linked_Posts::register_linked_post_type()
				'add_new'                 => esc_html_x( 'Add New', self::POST_TYPE_KEY, 'tribe-ext-speaker-linked-post-type' ),
				'add_new_item'            => esc_html__( 'Add New Speaker', 'tribe-ext-speaker-linked-post-type' ),
				'edit_item'               => esc_html__( 'Edit Speaker', 'tribe-ext-speaker-linked-post-type' ),
				'new_item'                => esc_html__( 'New Speaker', 'tribe-ext-speaker-linked-post-type' ),
				'view_item'               => esc_html__( 'View Speaker', 'tribe-ext-speaker-linked-post-type' ),
				'view_items'              => esc_html__( 'View Speakers', 'tribe-ext-speaker-linked-post-type' ),
				'search_items'            => esc_html__( 'Search Speakers', 'tribe-ext-speaker-linked-post-type' ),
				'not_found'               => esc_html__( 'No speakers found', 'tribe-ext-speaker-linked-post-type' ),
				'not_found_in_trash'      => esc_html__( 'No speakers found in Trash', 'tribe-ext-speaker-linked-post-type' ),
				'all_items'               => esc_html__( 'Speakers', 'tribe-ext-speaker-linked-post-type' ),
				'archives'                => esc_html__( 'Speaker Archives', 'tribe-ext-speaker-linked-post-type' ),
				'insert_into_item'        => esc_html__( 'Insert into speaker', 'tribe-ext-speaker-linked-post-type' ),
				'uploaded_to_this_item'   => esc_html__( 'Uploaded to this speaker', 'tribe-ext-speaker-linked-post-type' ),
				'items_list'              => esc_html__( 'Speakers list', 'tribe-ext-speaker-linked-post-type' ),
		...
		public function get_post_id_field_name() {
			return $this->get_post_type_label( 'singular_name' ) . '_ID'; // Speaker_ID
		}
		...
		private function get_order_meta_key() {
			$key = '_' . $this->get_post_id_field_name() . '_Order'; // _Speaker_ID_Order

			return sanitize_key( $key ); // _speaker_id_order
		}
		...
		/**
		 * Saves the speaker information passed via an event.
		 *
		 ...
		 *
		 * @param array  $data        The Speaker data.
		...
		public function create_new_post( $data, $post_status = 'publish' ) {
			...
			if (
				...
				$speaker_id = wp_insert_post( array_filter( $postdata ), true );

				if ( ! is_wp_error( $speaker_id ) ) {
					$this->save_meta( $speaker_id, $data );
		 ...
		 *
		 * @see Tribe__Extension__Speaker_Linked_Post_Type::event_edit_form_save_data()
		 * @see Tribe__Extension__Speaker_Linked_Post_Type::save_data_from_meta_box()
		 *
		 * @param int   $id   The speaker ID to update.
		 * @param array $data The speaker data.
		 *
		 * @return int The updated speaker post ID
		 */
		...
		public function set_post_type_in_parse_query( $query ) {
			...
			if (
				...
				$query->tribe_ext_is_event_speaker = true;
				...
				// Single Speaker Page: Force Previous and Next navigation links to not appear.
		...
		/**
		 * Override the Previous Events link on the Single Speaker page.
		 *
		 * @param $link
		 *
		 * @return string
		...
		/**
		 * Override the Next Events link on the Single Speaker page.
		 *
		 * @param $link
		 *
		 * @return string
		...
		protected function nothing_found_notice() {
			Tribe__Notices::set_notice( 'event-search-no-results', esc_html__( 'There were no results found.', 'tribe-ext-speaker-linked-post-type' ) );
		}
